...
import { generateWanVacePersonChangeOneVideo } from "@/api/generation";
import VideoHistoryGrid from "./VideoHistoryGrid";
import VideoLightbox from "./VideoLightbox";

const PAGE_SIZE = 15;
const PCO_MODEL_KEY = "angle_person_change_one_model";
const PCO_SIZE_KEY = "angle_person_change_one_size";
const PCO_FPS_KEY = "angle_person_change_one_fps";

const PCO_MODEL = "Wan2.2 Vace";

const VideoPersonChangeOneView = () => {
  ...
  const [size, setSize] = useState(() => localStorage.getItem(PCO_SIZE_KEY) || "576x1024");
  const [fps, setFps] = useState(() => localStorage.getItem(PCO_FPS_KEY) || "16");
  ...
  useEffect(() => {
    if (size != null) localStorage.setItem(PCO_SIZE_KEY, String(size));
  }, [size]);
  useEffect(() => {
    if (fps != null) localStorage.setItem(PCO_FPS_KEY, String(fps));
  }, [fps]);
  ...
  const loadHistory = async () => {
    ...
    try {
      const data = await getHistory("person_change_one");
  ...
  const handleGenerate = async () => {
    ...
    setRendering(true);
    try {
      await generateWanVacePersonChangeOneVideo({
        videoFile,
        image1File,
        image2File,
        width: w,
    ...
    <div className="k-top-section">
      <div className="k-top k-top-expand">
        <div className="k-expand-row">
          <div className="k-slots k-slots-3 k-slots-person-one">
            ...
            <label className="k-slot">
              ...
              <input type="file" accept="image/*" onChange={(e) => handleSelectImage2(e.target.files?.[0])} />
              {image2Preview && (
                <button type="button" className="k-slot-clear" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleClearImage2(); }}>
                  x
                </button>
              )}
            </label>
        ...
        <div className="k-prompt-wrap k-prompt-wrap-compact">
          <div className="k-options-row">
            <div className="k-model-select">
              ...
              <select value={PCO_MODEL} disabled aria-label="模型">
                <option value={PCO_MODEL}>{PCO_MODEL}</option>
              </select>
            </div>
...
export default VideoPersonChangeOneView;
