...
import { generateWanVacePersonChangeMixVideo } from "@/api/generation";
import VideoHistoryGrid from "./VideoHistoryGrid";
import VideoLightbox from "./VideoLightbox";

const PAGE_SIZE = 15;
const PCM_SIZE_KEY = "angle_person_change_mix_size";
const PCM_FPS_KEY = "angle_person_change_mix_fps";

const PCM_MODEL = "Wan2.2 Vace";

const VideoPersonChangeMixView = () => {
  const [videoFile, setVideoFile] = useState(null);
  const [videoPreviewUrl, setVideoPreviewUrl] = useState("");
  const [image1File, setImage1File] = useState(null);
  const [image1Preview, setImage1Preview] = useState("");

  const [size, setSize] = useState(() => localStorage.getItem(PCM_SIZE_KEY) || "840x1024");
  const [fps, setFps] = useState(() => localStorage.getItem(PCM_FPS_KEY) || "16");
  ...
  useEffect(() => {
    if (size != null) localStorage.setItem(PCM_SIZE_KEY, String(size));
  }, [size]);
  useEffect(() => {
    if (fps != null) localStorage.setItem(PCM_FPS_KEY, String(fps));
  }, [fps]);
  ...
  const loadHistory = async () => {
    ...
    try {
      const data = await getHistory("person_change_mix");
  ...
  const handleGenerate = async () => {
    ...
    const parts = (size || "840x1024").split("x").map((n) => parseInt(String(n).trim(), 10) || 0);
    const w = parts[0] > 0 ? parts[0] : 840;
    const h = parts[1] > 0 ? parts[1] : 1024;
    setRendering(true);
    try {
      await generateWanVacePersonChangeMixVideo({
        videoFile,
        image1File,
        width: w,
        height: h,
  ...
  const sizeParts = (size || "840x1024").split("x").map((n) => n.trim());
    ...
    <div className="k-top-section">
      <div className="k-top k-top-expand">
        <div className="k-expand-row">
          <div className="k-slots k-slots-2 k-slots-person-mix">
            ...
            <label className="k-slot">
              {image1Preview ? (
                <img src={image1Preview} alt="视频替换帧" style={{ width: "100%", height: "100%", objectFit: "contain" }} />
              ) : (
                <div className="k-slot-empty">
                  <strong>视频替换帧</strong>
                  <span>点击上传图片</span>
        ...
        <div className="k-prompt-wrap k-prompt-wrap-compact">
          <div className="k-options-row">
            <div className="k-model-select">
              ...
              <select value={PCM_MODEL} disabled aria-label="模型">
                <option value={PCM_MODEL}>{PCM_MODEL}</option>
              </select>
            </div>
...
export default VideoPersonChangeMixView;
