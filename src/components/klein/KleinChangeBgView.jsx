...
import { generateChangeBgLocal } from "@/api/generation";
import KleinHistoryGrid from "./KleinHistoryGrid";
import KleinLightbox from "./KleinLightbox";

const PAGE_SIZE = 24;
const MODEL_VERSION_KEY = "klein_changebg_model";

const KleinChangeBgView = () => {
  ...
  const [modelVersion, setModelVersion] = useState(
    localStorage.getItem(MODEL_VERSION_KEY) || "qwen_one"
  );
  ...
  const modelOptions = useMemo(
    () => [
      { label: "Qwen Edit 2509 One", value: "qwen_one" },
      { label: "Qwen Edit 2509 Double", value: "qwen_double" },
      { label: "Flux2 Klein One", value: "flux2_klein_one" },
      { label: "Flux2 Klein Double", value: "flux2_klein_double" },
    ],
    []
  );
  ...
  const loadHistory = async () => {
    ...
    try {
      const data = await getHistory("changebg");
  ...
  const handleGenerate = async () => {
    ...
    setRendering(true);
    try {
      const result = await generateChangeBgLocal({
        personFile,
        bgFile,
        model: modelVersion,
      });
  ...
  useEffect(() => {
    const handler = (event) => {
      ...
      if (message.type === "new_image" && message.data?.type === "changebg") {
...
export default KleinChangeBgView;
