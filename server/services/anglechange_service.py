...
MODEL_ENDPOINT_MAP = {
    "qwen_edit_2511": "/qwen_change_sight",
}
...
async def generate_anglechange(image, prompt: str, model: str):
    image_bytes = await image.read()
    image_b64 = file_to_base64(image_bytes)

    endpoint = MODEL_ENDPOINT_MAP.get(model, "/qwen_change_sight")
    ...
    filename = f"anglechange_{uuid.uuid4().hex[:8]}_{int(time.time())}.png"
    ...
    return {"images": [image_url], "type": "klein_angle"}
